		...
		<div class="vertical-align-wrap">
			<div class="vertical-align-middle">
				<div class="auth-box">
					<div class="content">
						<div class="header">
							<div class="logo align-center"><img width="100%" src="{{ asset('/assets/images/img/logo.png') }}" alt="Workday"></div>
							<p class="lead color-custom">{{ __('Sign in to your account') }}</p>
						</div>
						<form class="form-auth-small ui form" action="{{ route('login') }}" method="POST">
                       		@csrf
							<div class="fields">
								<div class="sixteen wide field {{ $errors->has('email') ? ' has-error' : '' }}">
									<label for="email" class="color-custom">{{ __('Email') }}</label>
							...
							<div class="fields">
								<div class="sixteen wide field {{ $errors->has('password') ? ' has-error' : '' }}">
									<label for="password" class="color-custom">{{ __('Password') }}</label>
							...
							<div class="fields">
								<div class="sixteen wide field">
									<div class="ui checkbox">
										...
										<label class="color-custom">{{ __('Remember me') }}</label>
									</div>
								</div>
							</div>
							<button type="submit" class="green-bg ui green button large fluid">{{ __('SIGN IN') }}</button>
						</form>
					</div>
				</div>
			</div>
